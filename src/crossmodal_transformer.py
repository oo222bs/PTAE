...
class ActionDecoderLayer(nn.Module):
    def __init__(self, params, embedding, appriori_length=True, vis_out=False, coll_out=False):
        ...
        if self.with_gripper:
            ...
            if vis_out:
                if self.coll_out:
                    self.linear = nn.Linear(self.d_model, self.params.VB_input_dim - 2)
                else:
                    self.linear = nn.Linear(self.d_model, self.params.VB_input_dim - 1)
            else:
                if self.coll_out:
            ...
            if vis_out:
                if self.coll_out:
                    ...
                    self.linear = nn.Linear(self.d_model, self.params.VB_input_dim)
            else:
                if self.coll_out:
                    self.linear = nn.Linear(self.d_model, self.params.B_input_dim-1)
                else:
