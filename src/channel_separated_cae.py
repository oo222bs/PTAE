...
def extract_visual_features(channel=0, dir='', opp=False):
    ...
    if channel == 0:
        ...
        if opp:
            model.load_state_dict(torch.load(train_conf.cae_save_dir_green + '/cae_model_green_opp_agent.pt'))
        else:
        ...
        if opp:
            model.load_state_dict(torch.load(train_conf.cae_save_dir_blue + '/cae_model_blue_opp_agent.pt'))
        else:
...
# Reconstruct images
def reconstruct(opp=False):
    # get the training configuration
    train_conf = TrainConfig()
    train_conf.set_conf("../train/train_conf.txt")

    if opp:
        im_data_dir = train_conf.IM_opp_dir_test
        cae_red_model_name = '/cae_model_red_opp_agent.pt'
        cae_green_model_name = '/cae_model_green_opp_agent.pt'
        cae_blue_model_name = '/cae_model_blue_opp_agent.pt'
    else:
        im_data_dir = train_conf.IM_dir_test
        cae_red_model_name = '/cae_model_red.pt'
        cae_green_model_name = '/cae_model_green.pt'
        cae_blue_model_name = '/cae_model_blue.pt'
    ...
    modelred.load_state_dict(torch.load(train_conf.cae_save_dir_red + cae_red_model_name))
    modelred.eval()
    modelgreen.load_state_dict(torch.load(train_conf.cae_save_dir_green + cae_green_model_name))
    modelgreen.eval()
    modelblue.load_state_dict(torch.load(train_conf.cae_save_dir_blue + cae_blue_model_name))
    modelblue.eval()
