...
def read_input_folder_three_cubes(images_path, channel, greyscale=False):
    all_file_list = []
    dir_list = []
    ...
    for file_list in all_file_list:
        ...
        #count += 1
    return np.asarray(resized_images), all_file_list
...
def extract_visual_features(channel=0, dir=''):
    ...
    model = CAE().to(device)
    if channel == 0:
        model.load_state_dict(torch.load(train_conf.cae_save_dir_red + '/cae_model_red_RL_cam_fixed.pt'))
        folder_name = "../train/visual_feature_extraction_red/"
    elif channel == 1:
        model.load_state_dict(torch.load(train_conf.cae_save_dir_green + '/cae_model_green_RL_cam_fixed.pt'))
        folder_name = "../train/visual_feature_extraction_green/"
    else:
        model.load_state_dict(torch.load(train_conf.cae_save_dir_blue + '/cae_model_blue_RL_cam_fixed.pt'))
        folder_name = "../train/visual_feature_extraction_blue/"
    #model.load_state_dict(torch.load(train_conf.cae_save_dir_red + '/cae_model_red_opp_agent.pt'))
    model.eval()
    #dates = ["201207", "201223", "210107", "210115", "210129", "210203"]
    dates = ["230130"]
    for date in dates:
        im_data_dir = dir + date
        #im_data_dir = "../target_RL_updated/image_train_unlbld/" + date
        #channel=0
        ...
        #for i in range(0, resized_input.shape[0], 2):
            ...
            #resized_input_x_left = torch.from_numpy(resized_input[i, :, :, :, :]).to(device)
            #resized_input_x_right = torch.from_numpy(resized_input[i+1, :, :, :, :]).to(device)
            with torch.no_grad():
                visual_features = model.extract_visual_features(resized_input_x)
                #visual_features_l = model.extract_visual_features(resized_input_x_left)
                #visual_features_r = model.extract_visual_features(resized_input_x_right)
                #visual_features = torch.cat((visual_features_l,visual_features_r), -1)
            date = filenames[i][0].split(os.path.sep)[-3]
            #date = filenames[i][0].split(os.path.sep)[-4]
            filename = filenames[i][0].split(os.path.sep)[-2]
            #filename = filenames[i][0].split(os.path.sep)[-3]
...
def reconstruct():
    ...
    resized_input_red, filenames_red = read_input_folder(im_data_dir+'/yellow-violet/', 0, False)
    resized_input_green, filenames_green = read_input_folder(im_data_dir+'/yellow-violet/', 1, False)
    resized_input_blue, filenames_blue = read_input_folder(im_data_dir+'/yellow-violet/', 2, False)
    # resized_input_red = resized_input_red.reshape(resized_input_red.shape[0],resized_input_red.shape[3],
    #                                               resized_input_red.shape[1], resized_input_red.shape[2])
    # resized_input_red = np.expand_dims(resized_input_red, axis=0)
    # resized_input_green = resized_input_green.reshape(resized_input_green.shape[0], resized_input_green.shape[3],
    #                                                   resized_input_green.shape[1], resized_input_green.shape[2])
    # resized_input_green = np.expand_dims(resized_input_green, axis=0)
    # resized_input_blue = resized_input_blue.reshape(resized_input_blue.shape[0], resized_input_blue.shape[3], resized_input_blue.shape[1],
    #                                             resized_input_blue.shape[2])
    # resized_input_blue = np.expand_dims(resized_input_blue, axis=0)
    resized_input_red = resized_input_red.reshape(resized_input_red.shape[0], resized_input_red.shape[1],
                                         resized_input_red.shape[4], resized_input_red.shape[2], resized_input_red.shape[3])
    resized_input_green = resized_input_green.reshape(resized_input_green.shape[0], resized_input_green.shape[1], resized_input_green.shape[4],
                                         resized_input_green.shape[2], resized_input_green.shape[3])
    resized_input_blue = resized_input_blue.reshape(resized_input_blue.shape[0], resized_input_blue.shape[1], resized_input_blue.shape[4],
                                         resized_input_blue.shape[2], resized_input_blue.shape[3])
    ...
    modelred.load_state_dict(torch.load(train_conf.cae_save_dir_red + '/cae_model_red_opp_agent.pt'))
    modelred.eval()
    modelgreen.load_state_dict(torch.load(train_conf.cae_save_dir_green + '/cae_model_green_opp_agent.pt'))
    modelgreen.eval()
    modelblue.load_state_dict(torch.load(train_conf.cae_save_dir_blue + '/cae_model_blue_opp_agent.pt'))
    modelblue.eval()
...
def train_for_channel(colour):
    ...
    if colour == 'red':
        ...
        channel = 0
        model_name = '/cae_model_red_RL_cam_fixed.pt'
    elif colour == 'green':
        save_dir = train_conf.cae_save_dir_green
        channel = 1
        model_name = '/cae_model_green_RL_cam_fixed.pt'
    else:
        save_dir = train_conf.cae_save_dir_blue
        channel = 2
        model_name = '/cae_model_blue_RL_cam_fixed.pt'
    if not os.path.exists(os.path.dirname(save_dir)):
        os.mkdir(os.path.dirname(save_dir))

    # get the dataset folder
    im_data_dir = train_conf.IM_dir
    resized_input, _ = read_input_folder_three_cubes(im_data_dir, channel)
    resized_input_batch = resized_input.reshape(resized_input.shape[0],resized_input.shape[3], resized_input.shape[1], resized_input.shape[2])
...
def concat():
    ...
    #    if act.split(' ')[-1] == "test":
    #        test.append(i)
    test = np.loadtxt('../target_RL_cam_fixed_56/test.txt')
    for date in dates:
...
def concat_alt():
    ...
    for date in dates:
        ...
        for i in range(144):
            ...
            if acts[i] == "PUSH-R" or acts[i] == "PULL-R" or acts[i] == "SLIDE-R-L" or acts[i] == "SLIDE-R-R" or acts[i] == "SLIDE-M-R":
                red_f = V_r.transpose((1,0,2))[i,:V_len[i],10:20]
                green_f = V_g.transpose((1, 0, 2))[i, :V_len[i], 10:20]
                blue_f = V_b.transpose((1, 0, 2))[i, :V_len[i], 10:20]
            else:
                red_f = V_r.transpose((1,0,2))[i,:V_len[i],:10]
                green_f = V_g.transpose((1, 0, 2))[i, :V_len[i], :10]
                blue_f = V_b.transpose((1, 0, 2))[i, :V_len[i], :10]
            concatenated_f = np.concatenate((red_f,green_f,blue_f), axis=-1)
            np.savetxt(save_name, concatenated_f, fmt="%.6f")
if __name__ == "__main__":
    #train()
    #reconstruct()
    #extract_visual_features(0, "../target_RL_cam_fixed_56/image/")
    #extract_visual_features(1, "../target_RL_cam_fixed_56/image/")
    #extract_visual_features(2, "../target_RL_cam_fixed_56/image/")
    concat()
    #concat_alt()
