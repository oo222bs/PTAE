...
import os
import numpy as np
from ptae import CrossmodalTransformer, CrossmodalTransformerDecoderTransformerResNet,  train, train_limited_data, validate
from torch.utils.tensorboard import SummaryWriter
from torch.utils.data import DataLoader
from torch.optim.lr_scheduler import MultiStepLR, ReduceLROnPlateau
from dataset import PairedNico2BlocksDataset, ImageDataset
...
def main():
    ...
    model = CrossmodalTransformerDecoderTransformerResNet(paramaters, app_length=app_length).to(device)
    ...
    model_name = 'cm_transformer_resnet_encoder_decoder_rlbench_pressbuttons'
    date = str(datetime.datetime.now()).split('.')[0]
    writer = SummaryWriter(log_dir='.././logs/'+model_name+date)  # initialize the writer with folder "./logs"

    # Load the trained model
    checkpoint = torch.load(save_dir + '/cm_transformer_resnet_encoder_decoder_rlbench_pressbuttons.tar')       # get the checkpoint
    ...
    #image_data_train = ImageDataset(train_conf.IM_dir)
    #image_data_test = ImageDataset(train_conf.IM_dir_test, True)
    ...
    else:  # separate the gripper dimension first and then normalise and merge
        training_data.B_bw = np.concatenate((normalise(training_data.B_bw[:,:,:-1], max_joint, min_joint) * training_data.B_bin[:,:,:-1], np.expand_dims(training_data.B_bw[:,:,-1], -1)), -1)
        training_data.B_fw = np.concatenate((normalise(training_data.B_fw[:,:,:-1], max_joint, min_joint) * training_data.B_bin[:,:,:-1], np.expand_dims(training_data.B_fw[:,:,-1], -1)), -1)
        test_data.B_bw = np.concatenate((normalise(test_data.B_bw[:,:,:-1], max_joint, min_joint) * test_data.B_bin[:,:,:-1], np.expand_dims(test_data.B_bw[:,:,-1], -1)), -1)
        test_data.B_fw = np.concatenate((normalise(test_data.B_fw[:,:,:-1], max_joint, min_joint) * test_data.B_bin[:,:,:-1], np.expand_dims(test_data.B_fw[:,:,-1], -1)), -1)
    ...
    train_dataloader = DataLoader(training_data, batch_size=batch_size, shuffle=True)#shuffle=False)
    ...
    for epoch in range(num_of_epochs):
        ...
        for input in train_dataloader:
            image_data = []
            for filename in input['L_filenames']:
                data_dir = train_conf.IM_dir + '/' + filename.split(os.path.sep)[-2] + '/' + \
                           filename.split(os.path.sep)[-1].split('.')[0]
                image_data.append(ImageDataset(data_dir).transformed_images)
            ...
            l, b, t, signal = train(model, input, optimiser, epoch_loss, paramaters, vis_out=False, appriori_len=app_length)#train_limited_data(model, input, optimiser, epoch_loss, paramaters, signal, vis_out=False)
        ...
        writer.add_scalar('Training Loss - Describe', np.mean(epoch_loss_describe), epoch)
        ...
        if train_conf.test and (epoch+1) % train_conf.test_interval == 0:
            ...
            for input in test_dataloader:
                image_data = []
                for filename in input['L_filenames']:
                    data_dir = train_conf.IM_dir_test + '/' + filename.split(os.path.sep)[-2] + '/' + \
                               filename.split(os.path.sep)[-1].split('.')[0]
                    image_data.append(ImageDataset(data_dir, True).transformed_images)
                input["images"] = image_data
                input["L_fw"] = input["L_fw"].transpose(0, 1)
                input["B_fw"] = input["B_fw"].transpose(0, 1).to(device)
                #input["V_fw"] = input["V_fw"].transpose(0, 1).to(device)
                ...
                l, b, t, signal = validate(model, input, epoch_loss_t, paramaters, vis_out=False, appriori_len=app_length)
                print("test")
                print("step:{} total:{}, language:{}, behavior:{}, signal:{}".format(step, t, l, b, signal))
                if signal == 'describe':
            ...
            writer.add_scalar('Test Loss - Describe', np.mean(epoch_loss_t_describe), epoch)
            writer.add_scalar('Test Loss - Execute', np.mean(epoch_loss_t_execute), epoch)

        # Save the model parameters at every log interval
        if (epoch+1) % train_conf.log_interval == 0:
            torch.save({'model_state_dict': model.state_dict(),
                        'optimiser_state_dict': optimiser.state_dict()},
                       save_dir + '/cm_transformer_resnet_encoder_decoder_rlbench_pressbuttons.tar')
